...
export default function AddProductPage() {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-gray-300 flex items-center justify-center p-6" style={{ fontFamily: "'DM Serif Display', Georgia, serif" }}>
        <div className="text-center max-w-md">
          ...
          {form.imagePreview && (
            <img src={form.imagePreview} alt="Product" className="w-32 h-32 object-cover rounded-2xl mx-auto mb-6 shadow-lg" />
          )}
          <h2 className="text-3xl font-bold text-stone-800 mb-2">Product Added!</h2>
          <p className="text-stone-500 mb-1 font-sans text-sm">{form.title}</p>
          <p className="text-2xl font-bold text-emerald-600 mb-6">${Number(form.price).toFixed(2)}</p>
          <button
            onClick={handleReset}
            className="bg-stone-800 text-white px-8 py-3 rounded-full text-sm font-sans font-medium tracking-widest uppercase hover:bg-stone-700 transition-colors"
          >
            Add Another
          </button>
        </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className={`w-full bg-white border rounded-xl px-4 py-3 text-stone-800 placeholder-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-stone-300 transition resize-none
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className={`w-full appearance-none bg-white border rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-300 transition pr-10
                      ${form.category ? "text-stone-800" : "text-stone-300"}
            ...
            {(form.title || form.category || form.price) && (
              <div className="rounded-2xl border border-stone-100 bg-white p-4 flex gap-4 items-center">
                {form.imagePreview
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-stone-800 font-semibold text-sm truncate">{form.title || "Product name"}</p>
                  <p className="text-stone-400 text-xs mt-0.5">{form.category || "No category"}</p>
                </div>
                <p className="text-stone-800 font-bold text-sm flex-shrink-0">
